...
async function upload(file){if(!file)return;try{const form=new FormData();form.append('file',file);log(`Enviando ${file.name}…`);const r=await fetch('./api/jobs',{method:'POST',body:form});const b=await r.json();if(!r.ok)throw new Error(b.detail||'upload falhou');job=b;$('#uploadMeta').textContent=`${job.original_filename} · ${job.id.slice(0,8)}`;log(`Job ${job.id} pronto para seleção de filamento.`);await loadModel();await refreshAce();$('#sliceBtn').disabled=!selectedSlot}catch(e){log(e.message)}}

async function refreshAce(){try{ace=await api(`jobs/${job.id}/ace`);selectedSlot=job.selected_slot;renderAce()}catch(e){log(`ACE: ${e.message}`)}}
function renderAce(){const root=$('#aceSlots');root.innerHTML='';for(const s of ace.normalized){const div=document.createElement('div');const supported=s.material_type==='PLA';div.className=`slot ${supported?'':'unsupported'} ${selectedSlot?.human_slot===s.human_slot?'selected':''}`;const color=s.rgb?`rgb(${s.rgb.join(',')})`:'#777';div.innerHTML=`<div class="swatch" style="background:${color}"></div><strong>Slot ${s.human_slot}</strong><div>${s.material_type||'desconhecido'}</div><small>${s.loaded===true?'carregado':s.loaded===false?'':'estado carregado desconhecido'}</small>`;if(supported)div.onclick=async()=>{try{job=await api(`jobs/${job.id}/slot`,jsonOptions({human_slot:s.human_slot}));selectedSlot=job.selected_slot;renderAce();recolor();$('#sliceBtn').disabled=false;log(`Slot ${s.human_slot} selecionado.`)}catch(e){log(e.message)}};root.appendChild(div)}if(!ace.normalized.some(s=>s.material_type==='PLA')){$('#sliceBtn').disabled=true;log('Nenhum slot PLA disponível.')};if(!selectedSlot&&ace.normalized.filter(s=>s.material_type==='PLA').length===1){selectedSlot=ace.normalized.find(s=>s.material_type==='PLA');renderAce();recolor();$('#sliceBtn').disabled=false}}
