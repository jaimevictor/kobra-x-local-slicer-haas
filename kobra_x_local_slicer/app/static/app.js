...
const safeCssColor = (rgb) => Array.isArray(rgb) && rgb.length===3 && rgb.every(v=>Number.isInteger(v)&&v>=0&&v<=255) ? `rgb(${rgb.join(',')})` : '#777';
...
async function health(){try{const h=await api('health');let text='configuração necessária',ok=false;if(h.snapshot_available){const p=await api('printer/state');text=p.online===true?`${p.status||'online'}${p.job?.name?` · ${p.job.name}`:''}`:'impressora offline';ok=p.online===true&&!p.stale}else if(h.integration_error)text='integração HA indisponível';else if(h.lan_transport_configured)text='aguardando Home Assistant';$('#printerStatus').textContent=text;$('#printerStatus').className=`status ${ok?'ok':'bad'}`;const c=await api('config');$('#configCard').classList.remove('hidden');if(c.printer_host)$('#printerHost').value=c.printer_host}catch(e){$('#printerStatus').textContent='erro';$('#printerStatus').className='status bad';log(e.message)}}

$('#discoverHa').onclick=async()=>{try{const list=await api('onboarding/discover');const root=$('#haCandidates');root.replaceChildren();for(const c of list){const div=document.createElement('div'),label=document.createElement('label'),radio=document.createElement('input'),name=document.createElement('strong'),meta=document.createElement('small');div.className='candidate';radio.type='radio';radio.name='haDevice';radio.value=String(c.device_id);name.textContent=String(c.name);meta.className='muted';meta.textContent=`${Number(c.entities?.length)||0} entidades${c.ace_device_id?' · ACE associado automaticamente':' · ACE não encontrado'} · resolução estável por translation_key`;radio.onchange=()=>{selectedHa=c};label.append(radio,name);div.append(label,meta);root.appendChild(div)}if(!list.length)throw new Error('Nenhum dispositivo anycubic_cloud foi encontrado no registry.');if(list.length===1){selectedHa=list[0];root.querySelector('input').checked=true}}catch(e){log(e.message)}};
$('#saveConfig').onclick=async()=>{try{if(!selectedHa)throw new Error('Selecione o dispositivo Anycubic descoberto no Home Assistant.');const body={printer_host:$('#printerHost').value.trim(),ha_device_id:selectedHa.device_id};const r=await fetch('./api/config',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});const b=await r.json();if(!r.ok)throw new Error(b.detail||'falha');await health();log('Configuração salva; entidades serão resolvidas automaticamente.')}catch(e){log(e.message)}};
...
function renderAce(){const root=$('#aceSlots');root.replaceChildren();for(const s of ace.normalized){const div=document.createElement('div'),swatch=document.createElement('div'),title=document.createElement('strong'),material=document.createElement('div'),detail=document.createElement('small');const supported=s.material_type==='PLA';div.className=`slot ${supported?'':'unsupported'} ${selectedSlot?.human_slot===s.human_slot?'selected':''}`;swatch.className='swatch';swatch.style.background=safeCssColor(s.rgb);title.textContent=`Slot ${Number(s.human_slot)||'?'}`;material.textContent=s.material_type||'desconhecido';detail.textContent=s.loaded===true?'carregado':s.loaded===false?'':'estado carregado desconhecido';div.append(swatch,title,material,detail);if(supported)div.onclick=async()=>{try{job=await api(`jobs/${job.id}/slot`,jsonOptions({human_slot:s.human_slot}));selectedSlot=job.selected_slot;renderAce();recolor();$('#sliceBtn').disabled=false;$('#sliceBtn').title='';$('#sliceState').textContent=`Slot ${s.human_slot} selecionado; pronto para fatiar`;log(`Slot ${s.human_slot} selecionado.`)}catch(e){log(e.message)}};root.appendChild(div)}if(!ace.normalized.some(s=>s.material_type==='PLA')){$('#sliceBtn').disabled=true;$('#sliceBtn').title='Nenhum slot PLA foi encontrado no ACE';log('Nenhum slot PLA disponível.')}if(selectedSlot)recolor()}
