...
async function refreshAce(){try{ace=await api(`jobs/${job.id}/ace`);job=await api(`jobs/${job.id}`);selectedSlot=job.selected_slot;renderAce();$('#sliceState').textContent=selectedSlot?'pronto para fatiar':'selecione um slot PLA';return true}catch(e){$('#aceSlots').innerHTML='<p class="muted">Não foi possível consultar o ACE.</p>';$('#sliceState').textContent=`ACE indisponível: ${e.message}`;log(`ACE: ${e.message}`);return false}}
