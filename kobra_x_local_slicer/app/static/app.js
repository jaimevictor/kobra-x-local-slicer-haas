...
$('#discoverHa').onclick=async()=>{try{const list=await api('onboarding/discover');const root=$('#haCandidates');root.innerHTML='';$('#haRoles').classList.add('hidden');for(const c of list){const div=document.createElement('div');div.className='candidate';const ace=c.ace_device_id?' · ACE associado automaticamente':' · ACE não encontrado';div.innerHTML=`<label><input type="radio" name="haDevice" value="${c.device_id}"><strong>${c.name}</strong></label><small class="muted">${c.entities.length} entidades${ace} · mapeamentos pendentes: ${c.unresolved_roles.join(', ')||'nenhum'}</small>`;const radio=div.querySelector('input');radio.onchange=()=>{selectedHa=c;renderHaRoles(c)};root.appendChild(div)}if(!list.length)throw new Error('Nenhum dispositivo anycubic_cloud foi encontrado no registry.');if(list.length===1){selectedHa=list[0];const radio=root.querySelector('input');radio.checked=true;renderHaRoles(selectedHa)}}catch(e){log(e.message)}};
$('#saveConfig').onclick=async()=>{try{if(!selectedHa)throw new Error('Selecione o dispositivo Anycubic descoberto no Home Assistant.');const body={printer_host:$('#printerHost').value.trim(),ha_device_id:selectedHa.device_id,ha_entity_map:selectedHaMap(),ha_ace_entity_map:selectedHa.ace_suggested_map||{}};const r=await fetch('./api/config',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});const b=await r.json();if(!r.ok)throw new Error(b.detail||'falha');await health();log('Configuração salva com mapeamento do ACE.')}catch(e){log(e.message)}};
