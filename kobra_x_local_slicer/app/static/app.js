...
async function health(){try{const h=await api('health');$('#printerStatus').textContent=h.printer_host_configured?(h.lan_connected?'LAN conectado':'LAN configurado'):'configuração necessária';$('#printerStatus').className=`status ${h.printer_host_configured?'ok':'bad'}`;const c=await api('config');$('#configCard').classList.remove('hidden');if(c.printer_host)$('#printerHost').value=c.printer_host}catch(e){$('#printerStatus').textContent='erro';$('#printerStatus').className='status bad';log(e.message)}}

$('#discoverHa').onclick=async()=>{try{const list=await api('onboarding/discover');const root=$('#haCandidates');root.innerHTML='';for(const c of list){const div=document.createElement('div');div.className='candidate';const ace=c.ace_device_id?' · ACE associado automaticamente':' · ACE não encontrado';div.innerHTML=`<label><input type="radio" name="haDevice" value="${c.device_id}"><strong>${c.name}</strong></label><small class="muted">${c.entities.length} entidades${ace} · resolução estável por translation_key</small>`;const radio=div.querySelector('input');radio.onchange=()=>{selectedHa=c};root.appendChild(div)}if(!list.length)throw new Error('Nenhum dispositivo anycubic_cloud foi encontrado no registry.');if(list.length===1){selectedHa=list[0];root.querySelector('input').checked=true}}catch(e){log(e.message)}};
