...
async def health(request: Request):
    ...
    health = {
        "ok": True,
        "printer_host_configured": bool(s.printer_host),
        "ha_device_configured": bool(s.ha_device_id),
        "integration_error": getattr(request.app.state, "integration_error", None),
        "lan_transport_configured": bool(svc(request).lan),
        "ha_accessible": False,
    ...
    if s.ha_device_id:
        try:
            ...
            health.update(
                {
                    "ha_accessible": snapshot.ha_connected,
                    "integration_resolved": snapshot.essential_entities_available,
            ...
            health["integration_error"] = str(exc)
    return health
...
async def discover_home_assistant():
    try:
        return await AnycubicHomeAssistantAdapter().discover()
    except Exception as exc:
        error(exc)
...
async def printer_state(request: Request):
    try:
        return await svc(request).printer_snapshot()
    except Exception as exc:
        error(exc)
...
async def printer_capabilities(request: Request):
    try:
        return (await svc(request).printer_snapshot()).capabilities
    except Exception as exc:
        error(exc)
...
async def printer_integration(request: Request):
    try:
        return await svc(request).integration_diagnostics()
    except Exception as exc:
        error(exc)
...
async def recent_jobs(request: Request):
    return svc(request).store.list()[:20]


@router.post("/jobs")
async def create(file: UploadFile = File(...), request: Request = None):
    try:
        return await svc(request).create_job(file)
    except Exception as exc:
        error(exc)
...
async def ace(job_id: str, request: Request):
    try:
        return await svc(request).ace(job_id)
    except Exception as exc:
        error(exc)
...
async def slot(job_id: str, body: SlotInput, request: Request):
    try:
        return await svc(request).select_slot(job_id, body.human_slot)
    except Exception as exc:
        error(exc)
...
async def orientation(job_id: str, body: OrientationInput, request: Request):
    try:
        job, preview = await svc(request).set_orientation(job_id, body.orientation)
        return {"job": job, "preview_file": preview}
    except Exception as exc:
        error(exc)
...
async def slice_job(job_id: str, request: Request):
    try:
        return await svc(request).slice(job_id)
    except Exception as exc:
        error(exc)
...
async def confirm(job_id: str, body: ConfirmInput, request: Request):
    try:
        return await svc(request).confirm(job_id, body.gcode_sha256, body.table_clear)
    except Exception as exc:
        error(exc)
...
async def print_job(job_id: str, request: Request):
    try:
        return await svc(request).print(job_id)
    except Exception as exc:
        error(exc)
...
async def pause_job(job_id: str, request: Request):
    try:
        return await svc(request).control(job_id, "pause")
    except Exception as exc:
        error(exc)
...
async def resume_job(job_id: str, request: Request):
    try:
        return await svc(request).control(job_id, "resume")
    except Exception as exc:
        error(exc)
...
async def cancel_job(job_id: str, request: Request):
    try:
        return await svc(request).control(job_id, "cancel")
    except Exception as exc:
        error(exc)
...
async def toolpath(job_id: str, request: Request):
    ...
    if not p.is_file():
        raise HTTPException(404, "toolpath unavailable")
    return FileResponse(p, media_type="application/json")


@router.get("/jobs/{job_id}/model")
async def model(job_id: str, request: Request, oriented: bool = False):
    job = svc(request).job(job_id)
    p = svc(request).store.job_dir(job_id) / (
        "oriented_preview.3mf" if oriented else job.input_filename
    )
    if not p.is_file():
