...
from app.core.models import (
    AceSlot,
    AceSnapshot,
    PrinterFaultSnapshot,
    PrinterJobSnapshot,
    PrinterSnapshot,
    PrinterThermalSnapshot,
)
...
CAPABILITY_DEFAULTS = {
    "telemetry_from_ha": False,
    "ace_from_ha": False,
    "pause_via_ha": False,
    "resume_via_ha": False,
    "cancel_via_ha": False,
    "local_file_list_via_ha": False,
    "local_upload_via_ha": False,
    "local_start_via_ha": False,
    "local_start_with_ace_via_ha": False,
    "cloud_upload_via_ha": False,
    "camera_via_ha": False,
}
...
def _find_entity(entities: list[dict[str, Any]], keys: tuple[str, ...]) -> str | None:
    ...
    for key in keys:
        for entity in entities:
            if _translation_key(entity) == key and isinstance(
...
def suggest_entity_map(
    ...
    roles = {
        ...
        "state": ("current_status", "job_state"),
        "filename": ("job_name",),
        "current_fault": ("job_failed",),
    }
    found = {
        role: entity_id
        for role, keys in roles.items()
        if (entity_id := _find_entity(entities, keys))
    }
    errors = [
        row["entity_id"]
        for row in entities
        if _translation_key(row) in {"job_failed", "last_error", "last_error_code"}
...
def suggest_ace_entity_map(entities: list[dict[str, Any]]) -> dict[str, str]:
    ...
    return {
        ...
        if (entity_id := _find_entity(entities, (key,)))
    }
...
class AnycubicHomeAssistantAdapter:
    ...
    async def _ws(self, commands: list[tuple[str, dict[str, Any]]]) -> list[Any]:
        try:
            ...
            raise HomeAssistantError(
                f"Home Assistant WebSocket unavailable: {exc}"
            ) from exc

    async def _registries(self) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
        devices, entities = await self._ws(
    ...
    async def discover(self) -> list[dict[str, Any]]:
        ...
        for device_id, rows in by_device.items():
            if not {_translation_key(row) for row in rows}.intersection(
                {"printer_online", "current_status", "is_available"}
            ):
                continue
            child_ids = [
                ident
                for ident, device in device_map.items()
                if device.get("via_device_id") == device_id and ident in by_device
            ]
            ace_ids = [
                ident
                for ident in child_ids
                if {_translation_key(row) for row in by_device[ident]}.intersection(
                    ACE_KEYS
                )
            ]
                    ...
                    "entities": [
                        {
                            ...
                            "translation_key": _translation_key(row),
                        }
                        for row in rows
                    ],
                    "suggested_map": mapping,
                    "unresolved_roles": unresolved,
                    "ace_device_id": ace_ids[0] if ace_ids else None,
                    "ace_device_ids": ace_ids,
    ...
    async def resolve(self) -> None:
        ...
        if self.printer_device_id not in device_ids:
            raise HomeAssistantError(
                "selected Anycubic printer device is no longer in the registry"
            )
        children = {
            d["id"]
            for d in devices
            if isinstance(d.get("id"), str)
            and d.get("via_device_id") == self.printer_device_id
        }
        rows = [
            row
            for row in entities
            if row.get("platform") == DOMAIN
            and row.get("device_id") in children | {self.printer_device_id}
        ]
        ...
        self.ace_device_ids = [
            ...
            if any(
                row.get("device_id") == ident and _translation_key(row) in ACE_KEYS
                for row in rows
            )
        ]
    ...
    async def _refresh_states(self) -> dict[str, dict[str, Any]]:
        ...
        try:
            ...
            by_entity = {
                row.get("entity_id"): row for row in all_states if isinstance(row, dict)
            }
            states = {
                key: by_entity[entity_id]
                for key, entity_id in self.entities.items()
                if isinstance(by_entity.get(entity_id), dict)
            }
        ...
        try:
            ...
            raise HomeAssistantError(
                f"Home Assistant state read failed: {exc}"
            ) from exc
        self._state_cache = states
        self.last_health_check_at = datetime.now(UTC)
        return dict(states)
            ...
            try:
                async with aiohttp.ClientSession() as session:
                    async with session.ws_connect(
                        ...
                        for ident, event_type in enumerate(
                            (
                                "state_changed",
    ...
    def capabilities(
        ...
        def usable(key: str) -> bool:
            return key in keys and _availability((states or {}).get(key)) == "available"
        ...
        result.update(
            {
                ...
                "pause_via_ha": usable("pause_print"),
                "resume_via_ha": usable("resume_print"),
                "cancel_via_ha": usable("cancel_print"),
                "local_file_list_via_ha": usable("file_list_local"),
    ...
    def _ace(self, states: dict[str, dict[str, Any]]) -> AceSnapshot | None:
        if not any(
            key in states for key in ("ace_loaded_slot", "ace_spools", "ace_slot_1")
        ):
            return None
        ...
        for number in range(1, 5):
            ...
            colors = (
                attrs.get("colors_hex")
                if isinstance(attrs.get("colors_hex"), list)
                else []
            )
            slots.append(
                AceSlot(
                    human_slot=_integer(attrs.get("slot")) or number,
                    protocol_slot_index=number - 1,
                    material_type=None if _unknown(value) else str(value).upper(),
                    rgb=_rgb(color_hex),
                    loaded=(number == loaded_slot) if loaded_slot is not None else None,
                    color_hex=color_hex if isinstance(color_hex, str) else None,
                    colors_hex=[str(x) for x in colors if isinstance(x, str)],
                    is_multi_color=attrs.get("is_multi_color")
                    if isinstance(attrs.get("is_multi_color"), bool)
                    else None,
                    sku=attrs.get("sku") if isinstance(attrs.get("sku"), str) else None,
                    spool_loaded=attrs.get("spool_loaded")
                    if isinstance(attrs.get("spool_loaded"), bool)
                    else None,
                    status=attrs.get("status")
                    if isinstance(attrs.get("status"), str)
                    else None,
                    edit_status=attrs.get("edit_status")
                    if isinstance(attrs.get("edit_status"), str)
                    else None,
                    consumables_percent=_number(attrs.get("consumables_percent")),
                    raw_state=None if value is None else str(value),
                )
            )
    ...
    async def snapshot(self, *, refresh: bool = False) -> PrinterSnapshot:
        ...
        return PrinterSnapshot(
            ...
            printer_device_id=self.printer_device_id,
            ...
            ha_connected=self.ws_connected or self.last_health_check_at is not None,
            essential_entities_available=essential,
            entity_availability=availability,
            online=_bool(_state(states.get("printer_online"))),
            available=_bool(_state(states.get("is_available"))),
            busy=_bool(_state(states.get("is_busy"))),
            status=None if _unknown(status) else str(status),
            job=PrinterJobSnapshot(
                name=None
                if _unknown(_state(states.get("job_name")))
                else str(_state(states.get("job_name"))),
                state=None
                if _unknown(_state(states.get("job_state")))
                else str(_state(states.get("job_state"))),
                progress=_number(_state(states.get("job_progress"))),
                current_layer=_integer(_state(states.get("job_current_layer"))),
                total_layers=_integer(_state(states.get("job_total_layers"))),
                elapsed_minutes=_number(_state(states.get("job_time_elapsed"))),
                remaining_minutes=_number(_state(states.get("job_time_remaining"))),
                eta=None
                if _unknown(_state(states.get("job_eta")))
                else str(_state(states.get("job_eta"))),
            ...
            thermal=PrinterThermalSnapshot(
                nozzle_current=_number(_state(states.get("curr_nozzle_temp"))),
                nozzle_target=_number(_state(states.get("target_nozzle_temp"))),
                bed_current=_number(_state(states.get("curr_hotbed_temp"))),
                bed_target=_number(_state(states.get("target_hotbed_temp"))),
            ),
            print_speed_pct=_number(_state(states.get("print_speed_pct"))),
            fan_speed_pct=_number(_state(states.get("fan_speed_pct"))),
            aux_fan_speed_pct=_number(_state(states.get("aux_fan_speed_pct"))),
            box_fan_level=_number(_state(states.get("box_fan_level"))),
            local_files=_list(_state(states.get("file_list_local"))),
            job_image_url=None
            if _unknown(_state(states.get("job_image_url")))
            else str(_state(states.get("job_image_url"))),
            ...
            fault=PrinterFaultSnapshot(
                ...
                if _unknown(_state(states.get("last_error_code")))
                else str(_state(states.get("last_error_code"))),
                last_error_message=None
                if _unknown(_state(states.get("last_error")))
                else str(_state(states.get("last_error"))),
                historical=bool(
                    _state(states.get("last_error_code"))
                    or _state(states.get("last_error"))
                ),
            ),
    ...
    async def press(self, action: str) -> dict[str, Any]:
        ...
        if key not in self.entities:
            raise HomeAssistantError(
                f"{action} is unsupported by the selected Anycubic integration"
            )
        ...
        try:
            async with httpx.AsyncClient(timeout=8) as client:
                response = await client.post(
                    ...
                    headers=headers,
                    json={"entity_id": self.entities[key]},
                )
                response.raise_for_status()
        except httpx.HTTPError as exc:
            raise HomeAssistantError(
                f"Home Assistant {action} command failed: {exc}"
            ) from exc
        return {
            "action": action,
            "entity_id": self.entities[key],
            "accepted_by_home_assistant": True,
        }
    ...
    async def diagnostics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "printer_device_id": self.printer_device_id,
            "ace_device_ids": self.ace_device_ids,
            "entities_resolved": self.entities,
            "entities_missing": [
                key for key in ESSENTIAL_KEYS if key not in self.entities
            ],
            "freshness": {"observed_at": snapshot.observed_at, "stale": snapshot.stale},
            "capabilities": snapshot.capabilities,
            "upload_transport": "DirectLanFileTransfer",
            "start_transport": "ValidatedLegacyLanStart",
