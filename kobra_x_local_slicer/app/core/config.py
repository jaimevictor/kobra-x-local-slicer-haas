...
class Settings:
    ...
    def load(cls) -> 'Settings':
        ...
        if path.is_file():
            raw=json.loads(path.read_text(encoding='utf-8')); s.printer_host=raw.get('printer_host',''); s.ha_device_id=raw.get('ha_device_id',''); s.ha_entity_map=raw.get('ha_entity_map')
            for key in ('slicing_timeout_seconds','retention_hours'):
                if key in raw: setattr(s,key,int(raw[key]))
        return s
    def save_config(self) -> None:
        self.data_dir.mkdir(parents=True,exist_ok=True); (self.data_dir/'config.json').write_text(json.dumps({'printer_host':validate_printer_host(self.printer_host),'ha_device_id':self.ha_device_id,'ha_entity_map':self.ha_entity_map or {}},indent=2),encoding='utf-8')
